...
{    
    ...
    [SerializeField] private int containersToPlace = 1;
    ...
    public override RewardData GetReward()
    {
        ...
        if (!_crane.ContainerGrabbed && containersPlaced > containersToPlace - 1)
        {
    ...
    private void Update()
    {
        ...
        if(_crane.ContainerGrabbed &&
            Mathf.Abs(container.position.y - currentTarget.y) < 0.001f &&
            Mathf.Abs(container.position.z - currentTarget.z) < 0.25f)
        {
            _crane.ReleaseContainer(environment);
            containersPlaced += 1;
            grabRewarded = false;
    ...
    public override void ResetEnvironment()
    {
        ...
        target.transform.position = firstContainerOnDeck.position - new Vector3(0, 0, 2.55f);
